...
<body class="hold-transition sidebar-mini layout-fixed">
  <div class="wrapper">
            ...
            <div class="card-body">
              <table class="table table-striped" data-sorting="true" name="satu" id="satu">
                <thead>
                  <tr>
                    <th>NIK</th>
                    <th data-breakpoints="xs sm md">Nama</th>
                    <th data-breakpoints="xs sm md">Nama Panggilan</th>
                    <th>Tanggal Lahir</th>
                    <th data-breakpoints="xs sm md">Alamat</th>
                    <th>Nomor Kontak</th>
                    <th>Pengalaman</th>
                ...
                <tbody>
                  ...
                  $ambilPraKaryawan = mysqli_query($koneksi, "SELECT * FROM karyawan
                    left join pengguna on pengguna.nik = karyawan.nik
                    WHERE pengguna.id_jabatan = 1005");
                    while ($tampilPra = mysqli_fetch_assoc($ambilPraKaryawan)) {
                      ...
                      echo "<td>" . $tampilPra['pengalaman'] . "</td>";
                      echo "</tr>";
                    }
                    ?>
    ...
    var stackedBarChartData = {
      labels: [
        <?php
        while ($tampilKaryawan = mysqli_fetch_assoc($ambilKaryawan)) {
          echo "'" . $tampilKaryawan['nama_panggilan'] . "',";
        }
        ?>
      ],
      datasets: [{
        ...
        data: [
          <?php
          while ($tampilRating = mysqli_fetch_assoc($hitungRating)) {
            echo intval($tampilRating['rating']) . ",";
          }
          ?>
        ],
        borderWidth: 1
      },
      {
        ...
        data: [
          <?php
          while ($tampilMobil = mysqli_fetch_assoc($hitungMobil)) {
            echo intval($tampilMobil['mobil']) . ",";
          }
          ?>
        ],
      ...
      },
      {
        ...
        borderWidth: 1,
        data: [<?php
        while ($tampilMotor = mysqli_fetch_assoc($hitungMotor)) {
          echo intval($tampilMotor['motor']) . ",";
        }
        ?>],
      },
    ]
