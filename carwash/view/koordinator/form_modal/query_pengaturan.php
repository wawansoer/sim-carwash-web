...
$ambilData = mysqli_query($koneksi, "SELECT * FROM karyawan
  ...
  $hasil [2][0] = $hasil [1][0] = $hasil [0][0] = $hasil [0][1]= "";
  if($jabatan === 'Karyawan'){
    $hitungCuci = mysqli_query($koneksi, "SELECT jenis.nama_jenis as jenis, COUNT(cuci.id_cuci) as jml FROM cuci
    ...
    WHERE cuci.nik = '$nik' GROUP by jenis.nama_jenis ORDER BY jenis.nama_jenis ");
    if(mysqli_num_rows($hitungCuci) > 0 ){
      $y = 0;
      while ($dataCuci = mysqli_fetch_assoc($hitungCuci)) {
        $hasil[$y][0] = $dataCuci['jenis'];
        $hasil[$y][1] = $dataCuci['jml'];
        $y++;
      }
    }
  }
